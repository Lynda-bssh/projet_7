...
## read data:
## Data without probability:
df_test = pd.read_csv('./data_eng/test.csv')
test = df_test[df_test.columns[df_test.columns != "SK_ID_CURR"]]
### Data with proba:
my_data = pd.read_csv("./prediction/data.csv")
## read data origin for visualisation:
df_origin  = pd.read_csv("./pretraitements_data/application_test.csv")
...
# une sidebar( logo de pret à depenser , button select, et 3 autres buttons) 
...
with header:
  ...
  ## La page principale:
   ...
   # image = Image.open('Sans titre.png')
    ...
    # # Une fois le chargement terminé, vous pouvez le cacher
    ...
    #     shap.force_plot(explainer_model.expected_value, shap_values_test[instance_idx], id_client.drop(columns=["SK_ID_CURR"], axis=1).iloc[instance_idx, :])
    ...
    ###--------------------------Partie features locale:---------------------------------###
    
    explainer_model = joblib.load('shap_values.model')
    shap_values_test = explainer_model.shap_values(test)
    ...
    def feature_local():
        ...
        if user_choice == '**Analyse locale de motifs**':
            ...
            st_shap(shap.force_plot(explainer_model.expected_value, shap_values_test[instance_idx],
    ...
    #         st_shap(shap.summary_plot(shap_values_test,id_client.drop(columns=["SK_ID_CURR"],axis = 1)))
